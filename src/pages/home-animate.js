...
const zoom = Math.min(6, Math.floor(minWindowSize / 160));
let isSpinning = true;
const TAU = Zdog.TAU;
...
function update() {
  ...
  if (isSpinning) {
    ...
    illo.rotate.x = everyOtherCycle
      ? 0
      : ((Math.cos(theta) * -0.5 + 0.5) * TAU * -1) / 8;
  }
